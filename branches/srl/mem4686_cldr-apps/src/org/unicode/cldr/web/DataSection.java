	...
	public class ExampleEntry {
		...
		public ExampleEntry(DataSection section, DataRow p, DataRow.CandidateItem item, CheckCLDR.CheckStatus status) {
			...
			hash = CookieSession.cheapEncode(DataSection.getN()) + // unique
					...
					// status..
					p.fullFieldHash(); /*
												 * fieldHash ensures that we
												 * don't get the wrong field..
												 */
		}
	}
...
//							"/standard/standard$");
	...
	private String sectionHash; // prefix string used for calculating html fields
                ...
                if (v != null) {
                    for (String avalue : v) {
                        ...
                        if (!avalue.equals(value)) {
                            ...
                            if (avalue!=null && (checkCldr != null)/* &&(altProposed == null) */) {
	...
	public void showPeasShort(WebContext ctx, int item_xpath) {
		...
		if (row != null) {
			...
			ctx.println("<tr><td colspan='2'>" + ctx.iconHtml("stop", "internal error")
					+ "<i>internal error: nothing to show for xpath " + item_xpath + "," + " "
					+ sm.xpt.getById(item_xpath) + "</i></td></tr>");
			// }
		}
	}
