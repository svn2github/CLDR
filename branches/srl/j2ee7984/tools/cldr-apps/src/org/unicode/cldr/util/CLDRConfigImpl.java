...
public class CLDRConfigImpl extends CLDRConfig implements JSONString {
    ...
    private synchronized void init() {
        if (isInitted)
            return;
        System.out.println("CLDRConfigImpl: Initting.");
        if (!cldrHomeSet) {
        ...
        // If this is set, load from env
        if(System.getenv("CLDR_MAINTENANCE") != null) {
            final Map<String,String> env = System.getenv();
            System.out.println("Loading ST settings from env..");
            for(Map.Entry<String, String> e : env.entrySet())
            {
                System.out.println("Set: " + e.getKey() + " = " + e.getValue());
                survprops.put(e.getKey(), e.getValue());
            }
        }
    ...
    private void createBasicCldr(File homeFile) {
        ...
        try {
            ...
            try (OutputStream file = new FileOutputStream(propsFile, false); // Append 
                PrintWriter pw = new PrintWriter(file);) {
                if(vcapMode()) {
                    pw.println("## VCAP mode - not writing anything.");
                    System.out.println("## VCAP mode");
                } else {
...
//            file.close();
    ...
    public static boolean vcapMode() {
