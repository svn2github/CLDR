...
public class TestSTFactory  extends TestFmwk {
	...
	public static void main(String[] args) {
		new TestSTFactory().run(args);
	}

	public TestSTFactory() {
		TestAll.setupTestDb();
	}
...
//		updcount= dbsrcfac.update();
		...
		for(String s : someLocales ) {
			CLDRLocale l = CLDRLocale.getInstance(s);
					logln("Loading: " + l);
			XMLSource src = dbsrcfac.makeSource(l.getBaseName(), false);
		...
		for(String s : someLocales ) {
			CLDRLocale l = CLDRLocale.getInstance(s);
					logln("Loading: " + l);
				XMLSource src = dbsrcfac.makeSource(l.getBaseName(), true);
				XMLSource srcF = dbsrcfac.makeSource(l.getBaseName(), false); // non vetted
		...
		for(String s : someLocales ) {
			CLDRLocale l = CLDRLocale.getInstance(s);
					logln("Loading: " + l);
					XMLSource src = dbsrcfac.makeSource(l.getBaseName(), true);
					XMLSource srcF = dbsrcfac.makeSource(l.getBaseName(), false);
...
//			logln("changed to (unvote), updates: " + updcount);
	...
	private STFactory getFactory() throws SQLException {
		if(gFac==null) {
			...
			//STFactory fac = new CLDRDBSourceFactory(sm, sm.fileBase, Logger.getAnonymousLogger(), cacheDir);
			logln("Setting up DB");
			STFactory fac = sm.stFactory;
